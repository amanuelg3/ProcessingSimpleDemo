...
void oscEvent(OscMessage msg){
  ...
  float x = msg.get(0).intValue();
  float y = msg.get(1).intValue();
  println("received x,y :"+ msg.toString());
 
  if(pattern.equals("/click")){
     arcball.remotePressed(x,y);
  }else if(pattern.equals("/drag")){
     arcball.remoteDragged(x,y);
  }
